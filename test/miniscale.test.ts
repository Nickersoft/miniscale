...
describe("scale() function", () => {
	const ms0 = scale(16, "major-second");
	const ms1 = scale(16, 1.125);
	const ms2 = scale(21, 1.2);
	const ms3 = scale(13.5, 1.5);

	it("resolves scale names correctly", () => {
		expect(ms0(0)).toMatchObject(ms1(0));
	});
	...
	it("returns the correct values when called with negative index", () => {
		...
		expect(ms2(-2)).toMatchObject({
			index: -2,
			ratio: 0.6944444444444445,
			value: 14.583333333333336,
		});
		expect(ms3(-7)).toMatchObject({
			index: -7,
	...
	it("includes values with units in the final result", () => {
		const ms = scale(10, 2); // 10, 20, 40, 80
...
describe("toArray()", () => {
	it("throws error when either min or max is undefined", () => {
		//@ts-expect-error
		expect(() => getArray(scale(16, 1.2))).toThrow();
		//@ts-expect-error
		expect(() => getArray(scale(16, 1.2), { min: 10 })).toThrow();
		//@ts-expect-error
		expect(() => getArray(scale(16, 1.2), { max: 10 })).toThrow();
	});

	it("throws error when min is not larger than zero", () => {
		expect(() => getArray(scale(16, 1.2), { min: 0, max: 100 })).toThrow();
		expect(() => getArray(scale(16, 1.2), { min: -1, max: 100 })).toThrow();
	});

	it("throws error when max is not larger than min", () => {
		expect(() => getArray(scale(16, 1.2), { min: 10, max: 10 })).toThrow();
		expect(() => getArray(scale(16, 1.2), { min: 2, max: 1 })).toThrow();
	});

	it("returns a scale as array, in the range of min and max", () => {
		const steps = getArray(scale(16, 1.125), {
			min: 14,
			max: 25,
		});
	...
	it("works when max is smaller than base", () => {
		const steps = getArray(scale(24, 2), { min: 3, max: 6 });
		expect(steps).toMatchObject([
	...
	it("works when min is larger than base", () => {
		const steps = getArray(scale(12, 2), { min: 24, max: 48 });
		expect(steps).toMatchObject([
	...
	it("includes values with units when called as getArray(withUnits(scale, min, max))", () => {
		const steps = getArray(scale(10, 2), { min: 2, max: 30 });
